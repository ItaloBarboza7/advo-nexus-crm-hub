...
import { Phone, Mail, User, Clock, DollarSign, RefreshCw, AlertCircle } from "lucide-react";
    ...
    return (
      <Card className="p-6">
        <div className="flex items-center gap-3 mb-4">
          ...
          <h3 className="text-lg font-semibold text-gray-900">Leads para Reconhectar</h3>
        </div>
    ...
    <Card className="p-6">
      ...
      <div className="mt-4 pt-4 border-t border-gray-200">
        <div className="flex justify-between items-center text-sm text-gray-600">
          <span>Total de oportunidades de recuperação: {recoverableLeads.length}</span>
          <span>Valor potencial: R$ {recoverableLeads.reduce((sum, lead) => sum + (lead.value || 0), 0).toLocaleString('pt-BR')}</span>
        </div>
      </div>
    </Card>
  );
