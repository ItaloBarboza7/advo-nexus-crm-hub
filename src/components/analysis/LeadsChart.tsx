...
export function LeadsChart({ leads, title, filterFunction, viewMode = 'weekly' }: LeadsChartProps) {
  console.log(`📊 LeadsChart "${title}" - renderizando com viewMode: ${viewMode}`);

  const chartData = useMemo(() => {
    const filteredLeads = filterFunction ? leads.filter(filterFunction) : leads;
    
    console.log(`📊 LeadsChart "${title}" - gerando dados para viewMode: ${viewMode}`);
  ...
  }, [leads, viewMode, filterFunction, title]);
