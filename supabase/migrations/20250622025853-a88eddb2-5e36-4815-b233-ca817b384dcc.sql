...
-- Corrigir o rastreamento de fechamento de contratos para usar auth.uid()
-- e garantir que os triggers existam nos esquemas de tenant

-- Atualizar a função principal para usar auth.uid()
...
-- Função para atualizar as funções de tenant existentes
CREATE OR REPLACE FUNCTION public.fix_closed_by_tracking_in_tenant(schema_name text)
RETURNS void
LANGUAGE plpgsql
SECURITY DEFINER
AS $$
BEGIN
  -- Atualizar função específica do tenant para usar auth.uid()
  EXECUTE format('
  ...
  IF EXISTS (
    ...
    EXECUTE format('DROP TRIGGER track_contract_closure_trigger ON %I.leads', schema_name);
  END IF;
...
-- Atualizar a função ensure_tenant_schema para usar auth.uid()
...
BEGIN
  ...
  IF NOT EXISTS (
    ...
    -- Criar outras tabelas... (mantendo o código existente)
    EXECUTE format('
...
-- Aplicar as correções aos esquemas de tenant existentes
DO $$
DECLARE
...
BEGIN
  ...
  LOOP
    -- Aplicar as correções a cada esquema de tenant
    PERFORM public.fix_closed_by_tracking_in_tenant(tenant_record.schema_name);
  END LOOP;
END;
$$;
