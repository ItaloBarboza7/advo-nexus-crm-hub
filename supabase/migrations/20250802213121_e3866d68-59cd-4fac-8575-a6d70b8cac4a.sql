...
-- First, let's fix the existing lead with incorrect status
UPDATE tenant_02d1e863_c22f_4568_8f17_c84b838e7e60.leads 
SET status = 'Proposta', updated_at = now() 
WHERE status = 'Propostathcna';

-- Create a function to update lead statuses when a column is renamed in tenant schema
...
BEGIN
  ...
  -- If column name changed, update all leads with the old status to the new status
  IF OLD.name IS DISTINCT FROM NEW.name THEN
    ...
    RAISE LOG 'Updated lead statuses from % to % in schema %', OLD.name, NEW.name, tenant_schema_name;
  END IF;
...
-- Add this trigger to existing tenant schemas
DO $$
DECLARE
...
BEGIN
  ...
  LOOP
    -- Add the trigger to each tenant schema
    EXECUTE format('
    ...
    RAISE LOG 'Added column rename trigger to schema: %', tenant_record.schema_name;
  END LOOP;
END;
$$;

-- Update the ensure_tenant_schema function to include this trigger for new schemas
...
BEGIN
  ...
  -- Ensure the column rename trigger exists (NEW ADDITION)
  IF NOT EXISTS (
    SELECT 1 FROM information_schema.triggers 
    WHERE trigger_schema = tenant_schema_name 
