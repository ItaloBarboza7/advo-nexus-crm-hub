...
-- Criar função para atualizar leads quando coluna do tenant for renomeada
CREATE OR REPLACE FUNCTION public.update_leads_on_tenant_column_rename()
RETURNS trigger
LANGUAGE plpgsql
AS $function$
DECLARE
...
BEGIN
  ...
  -- Se o nome da coluna mudou, atualizar todos os leads com o status antigo
...
-- Adicionar trigger para todas as tabelas kanban_columns existentes nos esquemas de tenant
DO $$
DECLARE
...
BEGIN
  ...
  LOOP
    -- Verificar se a tabela kanban_columns existe no esquema
    IF EXISTS (
      SELECT 1 FROM information_schema.tables 
      WHERE table_schema = tenant_schema_record.schema_name 
      AND table_name = 'kanban_columns'
    ) THEN
      -- Remover trigger existente se houver
      EXECUTE format('DROP TRIGGER IF EXISTS update_leads_on_column_rename_trigger ON %I.kanban_columns', tenant_schema_record.schema_name);
      ...
      EXECUTE format('
        CREATE TRIGGER update_leads_on_column_rename_trigger
          ...
          FOR EACH ROW
          EXECUTE FUNCTION public.update_leads_on_tenant_column_rename();
      ', tenant_schema_record.schema_name);
      
      RAISE LOG 'Created rename trigger for schema: %', tenant_schema_record.schema_name;
    END IF;
  END LOOP;
END;
$$;

-- Corrigir leads órfãos existentes (exemplo: "Propostababy" -> "Proposta")
DO $$
DECLARE
  tenant_schema_record RECORD;
BEGIN
  ...
  FOR tenant_schema_record IN 
    ...
    WHERE s.schema_name LIKE 'tenant_%'
  LOOP
    -- Verificar se existem leads com status que não correspondem às colunas atuais
    EXECUTE format('
      UPDATE %I.leads 
      SET status = ''Proposta'', updated_at = now()
      WHERE status = ''Propostababy''
        AND NOT EXISTS (
          SELECT 1 FROM %I.kanban_columns 
          WHERE name = ''Propostababy''
        )
    ', tenant_schema_record.schema_name, tenant_schema_record.schema_name);
    
    -- Outras correções similares podem ser adicionadas aqui
    EXECUTE format('
      UPDATE %I.leads 
      SET status = ''Proposta'', updated_at = now()
      WHERE status = ''Propostathcna''
        AND NOT EXISTS (
          SELECT 1 FROM %I.kanban_columns 
          WHERE name = ''Propostathcna''
        )
    ', tenant_schema_record.schema_name, tenant_schema_record.schema_name);
    ...
    EXECUTE format('
      UPDATE %I.leads 
      SET status = ''Proposta'', updated_at = now()
      WHERE status = ''Propostachouse''
        AND NOT EXISTS (
          SELECT 1 FROM %I.kanban_columns 
          WHERE name = ''Propostachouse''
        )
    ', tenant_schema_record.schema_name, tenant_schema_record.schema_name);
    
    RAISE LOG 'Fixed orphaned leads in schema: %', tenant_schema_record.schema_name;
  END LOOP;
END;
$$;

-- Atualizar a função ensure_tenant_schema para incluir o trigger de rename
...
BEGIN
  ...
  IF NOT EXISTS (
    ...
    -- Add the rename trigger for new schemas
    EXECUTE format('
      CREATE TRIGGER update_leads_on_column_rename_trigger
        AFTER UPDATE ON %I.kanban_columns
        FOR EACH ROW
  ...
  -- Ensure the rename trigger exists for existing schemas
  IF NOT EXISTS (
    SELECT 1 FROM information_schema.triggers 
    WHERE trigger_schema = tenant_schema_name 
