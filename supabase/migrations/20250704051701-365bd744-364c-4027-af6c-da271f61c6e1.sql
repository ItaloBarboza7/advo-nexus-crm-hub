...
-- Fix the exec_sql function to properly handle SELECT queries with leading whitespace/newlines
...
BEGIN
  ...
  -- Trim whitespace and newlines, then check if it's a SELECT statement
...
-- Test the function with a query that has leading whitespace to verify it works
SELECT public.exec_sql('
    SELECT 1 as test_column, ''test_value'' as test_text
');
